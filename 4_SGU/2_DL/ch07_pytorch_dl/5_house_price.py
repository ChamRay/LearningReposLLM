...
def create_dataset():
    # 1.从文件读取数据
    data = pd.read_csv("/Users/mac/workspace/LearningReposLLM/4_SGU/2_DL/data/house_prices.csv")
    # 2.去除无关列
    ...
    # 5.2.2 类别型特征：用默认值填充缺失项，再做独热编码处理
...
# 3. 自定义损失函数
def log_mse(y_pred,target):
    y_pred = torch.clamp(y_pred,1,float('inf'))
    mse = nn.MSELoss()
    return torch.sqrt(mse(torch.log(y_pred),torch.log(target)))

# 4. 模型训练
def train_test(model,train_dataset,test_dataset,lr,epoch_num,batch_size,device=torch.device('cpu')):
    # 1. 初始化相关操作
    ...
    for epoch in range(epoch_num):
        ...
        for batch_idx,(X,y) in enumerate(train_loader):
            ...
            y_pred = model(X)
            # 2.3.2 计算损失
            loss_value = log_mse(y_pred.squeeze(),y)
            # 2.3.3 反向传播
            loss_value.backward()
            # 2.3.4 更新参数
            optimizer.step()
